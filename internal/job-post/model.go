...
type JobPostModelInterface interface {
	...
	GetJobPostsWithUserID(id string, postType string, category, from, to, sort string) (*[]models.JobPost, error)
...
func (r *JobPostModel) GetJobPostsWithUserID(id string, postType string, category, from, to, sort string) (*[]models.JobPost, error) {
	collection := r.MongoClient.Database("cre-resume").Collection("jobPosts")

	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()

	typeFilter := bson.M{"ownerId": id, "type": postType}
